...
function Key({ size = '', accent = '' }) {
  const s = size ? `--${size}` : '';
  const a1 = accent === 'b' ? ' tk-face--b' : accent === 'o' ? ' tk-face--o' : '';
  const b1 = a1 === ' tk-face--b' ? ' tk-face--b3' : a1 === ' tk-face--o' ? ' tk-face--o3' : '';
  const b2 = a1 === ' tk-face--b' ? ' tk-face--b1' : a1 === ' tk-face--o' ? ' tk-face--o1' : '';
  const b3 = a1 === ' tk-face--b' ? ' tk-face--b2' : a1 === ' tk-face--o' ? ' tk-face--o2' : '';

  return (
    <div className={`tk-key tk-flex${size ? ` tk-key${s}` : ''}`}>
      <div className={`tk-key__front tk-face${size ? ` tk-key__front${s}` : ''}${b1}`} />
      <div className={`tk-key__back tk-face${size ? ` tk-key__back${s}` : ''}${b2}`} />
      <div className={`tk-key__right tk-face${size ? ` tk-key__right${s}` : ''}${b2}`} />
      <div className={`tk-key__left tk-face${size ? ` tk-key__left${s}` : ''}${b3}`} />
      <div className={`tk-key__top tk-face${size ? ` tk-key__top${s}` : ''}${b2}`} />
      <div className={`tk-key__bottom tk-face${size ? ` tk-key__bottom${s}` : ''}${b3}`} />
    </div>
  );
}
...
export function TypingKeyboard({
  ...
  useEffect(() => {
    ...
    const allKeys = kb.querySelectorAll('.tk-key');
    let alive = true;
    let idx = 0;
    let timer;
  ...
  return (
    <div className={['tk-container', className].filter(Boolean).join(' ')}>
      <style>{`
        ...
        .tk-keyboard__bottom { width: 500px; height: 160px; transform: rotateY(180deg) translateZ(12.5px); background-color: #EAE7E5; }
        ...
        .tk-key--down { display: flex; justify-content: center; align-items: center; transform: translateZ(-5px); transition: .05s ease; }
        .tk-key--down > .tk-key__top { background: color-mix(in srgb, ${secondaryAccent} 80%, white) !important; }
      `}</style>

      <div className="tk-main tk-flex">
