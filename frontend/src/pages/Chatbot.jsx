...
const Chatbot = () => {
  ...
  // === CTA: Build proposal handoff from structured projectState ===
  // Phase 4: removed regex budget detection and keyword-based division detection.
  // Now reads directly from projectState. Phase 8 will expand this into a full proposal brief.
  ...
  const handleConnectWithUs = () => {
    ...
    // Build a structured summary from the live projectState (Phase 8 will enrich this further)
    const lines = [];
    lines.push(`PROJECT PROPOSAL (auto-generated by TechBrahmand AI Architect on ${timestamp})`);
    lines.push('');
    ...
    if (s.totalCost) lines.push(`Budget: ${budget}`);
    lines.push('');
    lines.push('Note: Final scope and pricing will be confirmed after a detailed discussion with the TechBrahmand team.');
